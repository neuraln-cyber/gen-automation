...
# optimistic lock or make one terminal transition, and completion is accepted
# only when the latest decision set exactly meets the frozen target.
event.listen(
    ReviewDecision.__table__,
    "after_create",
    _ddl(
...
for _statement in (
    ...
    "WHERE review_task_id = OLD.id) <> OLD.desired_accepted_count "
    ...
    "SELECT CASE WHEN OLD.state IN ('succeeded', 'failed', 'cancelled') "
    "OR NEW.lock_version <> OLD.lock_version + 1 "
    "OR OLD.id IS NOT NEW.id "
...
for _statement in (
    ...
    "WHERE asset.id = NEW.watermark_asset_id "
    "AND asset.release_id = version.release_id AND asset.state = 'available' "
    "AND asset.storage_backend = NEW.watermark_storage_backend "
    "AND asset.storage_bucket = NEW.watermark_storage_bucket "
    "AND asset.object_key = NEW.watermark_object_key "
    "AND asset.object_version_id = NEW.watermark_object_version_id "
