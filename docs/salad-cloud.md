...
| `GEN_AUTOMATION_SALAD_OUTPUT_PROGRESS_WATCHDOG_SECONDS` | `600` (10 minutes). Only staged-runtime-admitted RUNNING jobs use this shorter bound. Each newly available raw master or exact current-intent cleanup resets the deadline; partial retries receive a bounded two-minute fallback grace per consecutive pre-existing output. Cancellation is durably recorded before the exact provider DELETE and retries only after Salad confirms the terminal state. |
