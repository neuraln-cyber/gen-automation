...
| Salad worker image pull | Read the selected immutable GPU-worker package. | Use a publicly readable package; no registry credential is stored by the application. | GPU canary. |

Only immutable `@sha256:<digest>` image references are deployable. The
control-plane package may remain private. The model-free GPU worker does not
bake models or LoRAs.
